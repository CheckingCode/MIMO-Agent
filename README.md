...
MiMo Agent 支持链式思维（Chain-of-Thought）推理显示，让你可以看到 AI 的推理过程：

- **实时推理显示**：在 AI 思考时，实时显示推理步骤和思考过程
- **可展开查看**：点击"思维链"按钮展开查看完整的推理过程
- **推理步骤统计**：显示推理步骤数量和字符数
- **默认启用**：默认开启思维链显示，可在设置中关闭

启用思维链显示，可以更好地理解 AI 如何分析问题和做出决策。
...
- 🧩 **Chain-of-Thought Reasoning**: View the AI's reasoning process in real-time with expandable thinking blocks showing step-by-step analysis.
